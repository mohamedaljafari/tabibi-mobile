...
} from "../lib/_e2e/provider-services";
import { readIncomingRequests, updateIncomingRequestStatus, type IncomingServiceRequest as ProviderRequest } from "../lib/_e2e/incoming-requests";
import {
  countUnreadNotifications as countProviderNotifications,
  readRecipientNotifications as readProviderNotifications,
} from "../lib/_e2e/notifications";
...
async function prepareProvider(): Promise<string> {
  ...
  expect(profile.account!.status).toBe("active");
...
describe("الدورة الكاملة للخدمة بين المريض ومقدم الخدمة", () => {
  ...
  it("الدردشة تفتح بعد القبول ويتبادل الطرفان الرسائل", async () => {
    ...
    // تبادل الرسائل
    await sendMessage(thread.threadId, "patient", "السلام عليكم، متى تصل؟");
    await sendMessage(thread.threadId, "provider", "وعليكم السلام، خلال 45 دقيقة");
    await sendMessage(thread.threadId, "patient", "تمام، في الانتظار");
    ...
    expect(await countUnreadMessages(thread.threadId, "patient")).toBeGreaterThanOrEqual(0);
  });
