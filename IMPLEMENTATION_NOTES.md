...
## المهمة الحالية: إصلاح انهيار تطبيق المريض APK على الجهاز (14-08-2026)
- المستخدم حمّل APK لتطبيق المريض (tabibi-mobile) وثبّته على جهاز أندرويد، لكن التطبيق يغلق فور الفتح ورسالة الخطأ تشير لعدم وجود استضافة/backend.
- السبب المرجح: التهيئة الافتراضية للقالب تحاول الاتصال بخادم خارجي عند الإقلاع:
  - `lib/_core/auth.ts` / `hooks/use-auth.ts` (useAuth يستدعي التوكن عبر secureStore)
  - `app/oauth/callback.tsx` + `constants/oauth.ts` (expo-oauth)
  - `lib/trpc.ts` + `lib/_core/api.ts` (trpc client يشير لخادم خارجي)
  - `app/_layout.tsx` قد يستدعي useAuth في المكون الجذر → إذا رمى استثناءً ينهار التطبيق بالكامل
- الحل المطلوب: حماية كل المسارات التي تعتمد على الخادم في الإقلاع بـ try/catch أو تعطيلها حتى يصبح التطبيق محليًا بالكامل، ثم التأكد من عمل التطبيق في محاكاة الأندرويد/الجهاز.
- تطبيق الشريك tabibi-partner سيتأثر بالمشكلة نفسها إذا كان مبنيًا من القالب نفسه — قد يحتاج الحل المماثل لاحقًا.
- النسخة المنشورة للمريض: https://tabibiapp-wd5gbxre.manus.space
- GitHub repos: tabibi-mobile (المريض) + tabibi-partner (الشريك) على حساب المستخدم shraf95alhusin.
- todo.md أُضيفت له 3 بنود: فحص سبب الانهيار / إصلاح / اختبار + checkpoint.
- بعد الإصلاح: شرح خطوات البناء الصحيح (eas أو Publish من واجهة الإدارة) — تذكير: APK يُبنى تلقائيًا من Publish بعد checkpoint.

### نتائج فحص سبب الانهيار (مكتمل التحليل):
1. **app/_layout.tsx سطر 66**: `const [trpcClient] = useState(() => createTRPCClient());` يُنفَّذ على كل المنصات بما فيها native.
2. **lib/trpc.ts**: `httpBatchLink({ url: `${getApiBaseUrl()}/api/trpc` })` — على native يكون `getApiBaseUrl() = ""` (لا EXPO_PUBLIC vars على الجهاز) فيصير url = `/api/trpc` نسبي → fetch إلى `/api/trpc` على الجهاز ينهار برفض اتصال أو خطأ URL غير صالح.
3. **lib/_core/api.ts**: `getApiBaseUrl()` على native يعيد ""، وurl = endpoint فقط (نسبي) — إن لم يُستدعَ apiCall على native فلا مشكلة.
4. **hooks/use-auth.ts**: على native يقرأ SecureStore/AsyncStorage فقط (جلسة OAuth) — لا اتصل بالخادم إذا لا جلسة. لكن useAuth غير مستخدم في أي شاشة الآن.
5. **الخلاصة**: الانهيار عند الفتح سببه إنشاء tRPC client بـ URL نسبي خاطئ ثم أول طلب trpc أو خطأ في الاتصال أثناء الإقلاع.
6. **الحل**: في lib/trpc.ts إذا Platform.OS !== "web" لا تستخدم httpBatchLink؛ استخدم httpLink بـ URL صالح أو أوقف العميل. الأسلم: إنشاء عميل «dummy» على native برابط وهمي لن يُستخدم أصلًا (لأن لا تفاعلات tRPC في التطبيق حاليًا) أو استخدام `httpBatchLink` فقط على web مع `createTRPCClient` آمن.
7. لا توجد شاشات تستخدم trpc hooks حاليًا — التطبيق محلي بالكامل.
8. app.config.ts موجود بدل app.json (expo-router يولّد app.json من config).

### نتائج الفحص النهائية لإصلاح انهيار APK (محدثة):
1. **السبب الرئيسي**: `lib/trpc.ts` — `createTRPCClient()` في `app/_layout.tsx` سطر 66 يعمل على native بـ `httpBatchLink({ url: "/api/trpc" })` لأن `getApiBaseUrl()` يعيد "" على native → fetch إلى URL نسبي `/api/trpc` يفشل/ينهار.
2. **الحل المطبق**: عدّل `lib/trpc.ts` — إذا `getApiBaseUrl()` فارغ (native) استخدم `NATIVE_STUB_URL = "https://localhost:0/api/trpc"` كبديل آمن (client عديم الضرر على native).
3. **باقي المكونات آمنة على native**:
   - `initManusRuntime()`: يتحقق `!isWeb() || !isInIframe()` ويعود فورًا.
   - `use-auth.ts`: على native يقرأ SecureStore فقط، لا يتصل بالخادم إذا لا جلسة (ولا يُستخدم في أي شاشة حاليًا).
   - `oauth/callback.tsx`: شاشة لا تُفتح إلا عبر deep link بعد OAuth — لا تسبب انهيارًا عند الفتح العادي.
   - لا توجد أي شاشة تستخدم `trpc.useQuery` أو `apiCall` — التطبيق محلي بالكامل.
4. **expo-notifications**: plugin فقط في app.config.ts + permissions، لا استدعاءات `Notifications.*` في app.
5. **متبقي**: إعادة تشغيل Metro للتحقق من عمل التطبيق على web + اختبار vitest + نقطة تحقق.

### تحديث حالة المهمة (إصلاح انهيار APK) — بعد شرح المستخدم:
- المستخدم أوضح أن الخطأ لم يظهر كنص، فقط انغلق التطبيق فور الفتح. الاستنتاج التقني صحيح: `getApiBaseUrl()` يعيد "" على native → `httpBatchLink` برابط نسبي يسبب الانهيار.
- **الحل المطبق**: lib/trpc.ts يستخدم NATIVE_STUB_URL ("https://localhost:0/api/trpc") على native.
- pnpm check: 0 أخطاء. vitest: 157 ناجحًا + 1 متخطى.
- اللقطة أكدت عمل التطبيق على web (صفحة تسجيل الدخول تظهر بالشعار).
- **ملاحظة**: في اللقطة الأخيرة ظهرت «الاسم الكالام» في عنوان الحقل — يجب التحقق: هل هذا خطأ نصي في login.tsx أم فقط في اللقطة القديمة؟ (المستخدم أشار سابقًا لإصلاح إملائي «الاسم الكامل»).
- متبقي: إصلاح «الاسم الكالام» إن وجد، حفظ نقطة تحقق، وتسليم APK جديد مع شرح خطوات النشر.
- طريقة النشر للمستخدم: زر Publish في واجهة إدارة المشروع ينتج APK الأندرويد تلقائيًا (15-30 دقيقة).

### تحقق نهائي (لقطة /login مقاس هاتف 375x812):
النص يظهر صحيحًا «الاسم الكامل» في الكود واللقطات (اللقطات السابقة كانت من نسخة قديمة قبل الـ refresh). الحل النهائي لإصلاح انهيار APK: lib/trpc.ts يستخدم NATIVE_STUB_URL على native بدل رابط نسبي فارغ. 157 اختبارًا ناجحًا، 0 أخطاء TS. متبقي: تحديث todo + نقطة تحقق + تسليم APK.

### فحص تطبيق الشريك (tabibi-partner) لإصلاح انهيار APK — المرحلة الحالية:
1. **نتيجة حاسمة**: تطبيق الشريك **لا يحتوي** `lib/trpc.ts` ولا أي عميل tRPC — `app/_layout.tsx` لا يستورد tRPC إطلاقًا (فقط QueryClient + Stack + SafeArea).
2. `_layout.tsx` نظيف على native: لا `initManusRuntime`، لا `useAuth`، لا استدعاءات apiCall عند الإقلاع.
3. المتبقي فحصه في الشريك: `hooks/use-auth.ts`، `oauth/callback.tsx`، `lib/_core/api.ts` (getApiBaseUrl)، `app.config.ts` (expo-notifications plugin)، و`vitest.config.ts`.
4. تطبيق المريض تَم إصلاحه: lib/trpc.ts يستخدم NATIVE_STUB_URL ("https://localhost:0/api/trpc") على native. نقطة تحقق f4602099.
5. خطة الشريك: إن كان لا يحتوي مشكلة tRPC أصلًا، فالانهيار قد يكون من شيء آخر — فحص use-auth.ts وoauth/ وexpo-notifications ثم تشغيل vitest + pnpm check + لقطة web للتأكد، ثم رفع GitHub وحفظ.
6. ملاحظة: مجلد الشريك /home/ubuntu/tabibi-partner موجود محليًا بالكامل (node_modules موجود) — لا حاجة لاستنساخ.


---

## فحص تطبيق الشريك (tabibi-partner) — 2026-08-15 — إصلاح انهيار APK

**النتيجة الرئيسية**: تطبيق الشريك **لا يحتوي عميل tRPC أصلاً** (لا يوجد lib/trpc.ts كعميل، لا tRPC في app/_layout.tsx)، ولا يستدعي apiCall عند الإقلاع (لا useAuth في app/ ولا initManusRuntime).

- hooks/use-auth.ts موجود لكنه مخصص للويب فقط (يفحص Platform !== web) ولا يستخدمه أي شاشة عند الإقلاع — لكن **استيراده قد يكون سببًا للانهيار** إن وُجدت import مبكرة.
- apiCall يعتمد على fetch → URL نسبي على native عندما getApiBaseUrl() يرجع "" → fetch("/api/...") على native **سينهار** (react-native لا يدعم fetch على URL نسبي).
- **المخاطر المتبقية للشريك على APK**:
  1. لا يوجد عميل tRPC → مشكلة تطبيق المريض السابقة غير موجودة هنا.
  2. أي شاشة تستخدم apiCall مباشرة على native ستنهار (يجب التأكد من عدم استخدام apiCall في شاشات الشريك الأساسية).
- الخلاصة العملية: تطبيق الشريك أأمن من تطبيق المريض على native، لكن يجب التأكد من عدم استدعاء apiCall بـ URL نسبي في أي تدفق مستخدم شائع.

**الإجراء المطبق**: لا حاجة لنسخة tRPC stub في الشريك (لا يوجد tRPC عميل). التحقق من عدم استخدام apiCall في شاشات الإقلاع فقط.


### فحص حاسم إضافي (tabibi-partner)

- **لا توجد أي شاشة في تطبيق الشريك تستخدم apiCall** (grep على app/ بلا نتائج).
- **لا توجد أي شاشة تستدعي useAuth أو initManusRuntime عند الإقلاع** — hooks/use-auth.ts موجود لكنه لم يُستخدم في app/.
- التطبيق كله (تسجيل، طلبات، دردشة) يستخدم AsyncStorage + مكتبات lib/ المحلية مباشرة.
- **النتيجة**: تطبيق الشريك لا ينهار من مشكلة الخادم أصلًا على native — لا يوجد اتصال بالخادم في أي مسار مستخدم. أي انهيار على APK في الشريك سيكون من سبب آخر (مثل splash/assets أو native modules).
- lib/_core/api.ts يوجد (السطر 39-44 يستخدم fetch) لكنه غير مستدعى في أي شاشة — مجرد ملف قوالب من القالب الأصلي.

### الخلاصة للمستخدم

تطبيق الشريك لا يحتاج تعديل trpc لأنه لا يستخدم tRPC إطلاقًا. الانهيار الذي رآه المستخدم كان على تطبيق المريض (طبيبي) وقد أُصلح بنقطة التحقق f4602099. يجب نشر تطبيق المريض للحصول على APK سليم؛ ثم تجربة الشريك لاحقًا — إن انهار الشريك أيضًا فسنفحصه سببًا سببًا (ربما plugin معين في app.config.ts).


### حالة تطبيق الشريك (لحظة الفحص)

عند الترقية السابقة لتجزئة كلمات المرور في /home/ubuntu/tabibi-partner/lib/provider-auth.ts:
- خطأ TS في السطر 287 والسطر 394: `Type 'string' is not assignable to type 'number'` — نوع passwordHash في ProviderAccount ما زال `number` في lib/provider-registry.ts للمشروع الشريك، ويجب توسيعه ليقبل `string | number` (كما فعلنا في تطبيق المريض).
- الاختبارات: 7 فاشلة من 31 (على الأرجح بسبب نفس المشكلة).
- الحل: تعديل lib/provider-registry.ts في tabibi-partner: passwordHash: string | number، ثم إعادة الاختبارات.
- ملاحظة مهمة: تطبيق الشريك لا يستخدم tRPC إطلاقًا — لا ينهار من مشكلة الخادم على native. انهيار APK الذي شاهده المستخدم كان على تطبيق المريض (طبيبي) وأُصلح في نقطة التحقق f4602099 — يجب نشره للحصول على APK سليم.
