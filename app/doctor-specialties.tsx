...
export default function DoctorSpecialtiesScreen() {
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View><Text style={styles.title}>اختر التخصص</Text><Text style={styles.subtitle}>اختر التخصص الطبي المناسب لك</Text></View>
        </View>

        <View style={styles.intro}><View style={styles.introIcon}><MaterialIcons name="medical-services" size={27} color="#6B7B3F" /></View><View style={styles.introCopy}><Text style={styles.introTitle}>أطباء حسب التخصص</Text><Text style={styles.introText}>بعد الاختيار، ستظهر لك صفحة البحث الخاصة بهذا التخصص.</Text></View></View>

        <View style={styles.grid}>
          {DOCTOR_SPECIALTIES.map((specialty) => (
            <Pressable key={specialty.id} accessibilityRole="button" accessibilityLabel={`اختيار ${specialty.title}`} onPress={() => router.push({ pathname: "/doctor-search", params: { specialtyId: specialty.id } } as never)} style={({ pressed }) => [styles.specialtyCard, pressed && styles.pressed]}>
...
const styles = StyleSheet.create({
  ...
  intro: { alignItems: "center", backgroundColor: "#EFF2E6", borderRadius: 18, flexDirection: "row-reverse", gap: 11, marginTop: 23, padding: 14 },
