...
export default function AssistedServiceSearchScreen() {
  const { serviceId } = useLocalSearchParams<{ serviceId?: string }>();
  const service = getAssistedService(serviceId);
  const [profile, setProfile] = useState<PatientProfile | null>(null);
  const [selectedAddress, setSelectedAddress] = useState<PatientAddress | null>(null);
  const [addressMenuOpen, setAddressMenuOpen] = useState(false);
  const [filterOpen, setFilterOpen] = useState(false);
  const [sort, setSort] = useState<AssistedServiceSort>("nearest");
  ...
  const providers = useMemo(() => getAssistedServiceProviders(service.id, sort), [service.id, sort]);
  const providerResults = useMemo(() => {
    if (!loaded) return [];
    return mergeAssistedServiceProviders(accounts, service.id);
  ...
  const chooseSort = (option: AssistedServiceSort) => { setSort(option); setFilterOpen(false); };

  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]}>
      <View style={styles.page}>
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.header}><Pressable accessibilityRole="button" accessibilityLabel="رجوع إلى الخدمات الطبية المساعدة" onPress={() => router.back()} style={({ pressed }) => [styles.backButton, pressed && styles.pressed]}><MaterialIcons name="arrow-forward" size={22} color="#6B7B3F" /></Pressable><View style={styles.headerCopy}><Text style={styles.title}>طلب {service.title}</Text><Text style={styles.subtitle}>اختر عنوان الزيارة وابحث عن الأقرب إليك</Text></View></View>
